...
impl TransactionHistories {
    ...
    pub(super) async fn new() -> anyhow::Result<Self> {
        ...
        let connection_option = sqlx::ConnectOptions::log_slow_statements(
            connection_options,
            log::LevelFilter::Warn,
            tokio::time::Duration::from_secs(3),
        );
        let connection_option = sqlx::ConnectOptions::log_statements(
            connection_option,
    ...
    pub(super) async fn get_by_account(
        ...
        for row in result_db {
            let tx_data = <super::RelatedTransaction as sqlx::FromRow<
                '_,
